"use strict";var username;var currentGames={};var roomList={};var createGameOptions={'name':'-','ruleset':'normal',};var inPreGame=false;var gameID=false;var gameState={};var colorMapping={'blue':'b','green':'g','yellow':'y','red':'r','purple':'p','black':'k','rainbow':'m',}
function preload(imageList){for(var i=0;i<imageList.length;i++){var image=new Image();image.id='card-'+imageList[i];image.src='/public/img/cards/'+imageList[i]+'.png';$('#card-images').append(image);}}
...
function hanabiShow(){console.log('Started game #'+gameID+'.');$('#pregame').fadeOut();$('#main-menu').fadeIn();$('#lobby').fadeOut(400,function(){$('#hanabi').fadeIn();});hanabiResizeCanvas();}
...
var hand_x=hand_pos[nump][j].x*win_w;var hand_y=hand_pos[nump][j].y*win_h;var hand_width=hand_pos[nump][j].w*win_w;var hand_height=hand_pos[nump][j].h*win_h;var hand_size;if(nump===2||nump===3){hand_size=5;}else if(nump===4||nump===5){hand_size=4;}else{errorShow('The number of players in the game is invalid.');}
for(var cardNum=0;cardNum<hand_size;cardNum++){var card=gameState.hands[i].cards[cardNum].card;var imageID;if(card.color==='unknown'&&card.number===0){imageID='card-back';}else{imageID='card-'+colorMapping[card.color]+card.number;}
var image=document.getElementById(imageID);context.drawImage(image,hand_x+(hand_width/5*cardNum),hand_y,hand_width/(hand_size*1.075),hand_height);}}
context.rect(.8*win_w,.6*win_h,.2*win_w,.4*win_h);context.stroke();context.rect(.183*win_w,.3*win_h,.435*win_w,.189*win_h);}
