...
- on button click, unbind event handler so that it doesn't send double websocket message
*/
...
function hanabiDraw() {
	...
	for (var i = 0; i < nump; i++) {
		...
		var hand_x = hand_pos[nump][j].x * win_w;
		var hand_y = hand_pos[nump][j].y * win_h;
		var hand_width = hand_pos[nump][j].w * win_w;
		var hand_height = hand_pos[nump][j].h * win_h;
		var hand_size;
		if (nump === 2 || nump === 3) {
			hand_size = 5;
			...
			errorShow('The number of players in the game is invalid.');
		}

		for (var cardNum = 0; cardNum < hand_size; cardNum++) {
			var card = gameState.hands[i].cards[cardNum].card;
			var imageID;
			if (card.color === 'unknown' && card.number === 0) {
				imageID = 'card-back';
			} else {
				imageID = 'card-' + colorMapping[card.color] + card.number;
			}
			var image = document.getElementById(imageID);
			context.drawImage(
				image,
				hand_x + (hand_width / 5 * cardNum),
				hand_y,
				hand_width / (hand_size * 1.075),
				hand_height
			);
		}
	}
